...
public class A_ShouHuoDiZhi_Present extends BasePresent<A_ShouHuoDiZhi_View> {
    ...
    public void adminCuseraddressPage(Integer current, Integer size, Context context) {
        ...
        Observable<ResponseBean<Cuseraddress_Total_Info>> observable = RetrofitClient.builderRetrofit(context).create(APIService_AJiTai.class).adminCuseraddressPage(current, size);
